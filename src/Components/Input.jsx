...
const Input = ({handleChange,handleSelect}) => {
  return (
    <div className='flex flex-col justify-between pl-4 desktop:flex-row desktop:px-20 bg-White dark:bg-Very_Dark_Blue pb-4'>
      ...
      <input type="text" name="country" className='rounded-lg drop-shadow-md shadow-inner px-4 py-4 w-11/12 h-20 desktop:h-auto border accent-Dark_Gray border-Dark_Gray text-Dark_Gray caret-Dark_Gray dark:bg-Dark_Blue dark:text-White dark:caret-White dark:placeholder:text-White dark:border-Dark_Blue dark:accent-Dark_Blue desktop:bg-search_light_mode desktop:bg-no-repeat desktop:bg-White desktop:bg-[center_left_1rem] desktop:bg-opacity-100 desktop:pl-[3.3rem] desktop:dark:bg-search_dark_mode cursor-pointer' placeholder='Search for a country... ' onChange={(event)=> handleChange(event.target.value)} />
      </div>
